...
def run_sensitivity_analysis(
    ...
    print(
        f"  All HR < 1.0 (treatment benefit): {(df_results['hazard_ratio'] < 1.0).all()}"
    )
    ...
    print(f"\nMedian Survival Times:")
    print(
        f"  Treatment: {df_results['median_treatment'].mean():.1f} months "
        f"(SD: {df_results['median_treatment'].std():.1f})"
    )
    print(
        f"  Control: {df_results['median_control'].mean():.1f} months "
    ...
    if hr_cv > 20:
        stability_issues.append(
            f"HR coefficient of variation ({hr_cv:.1f}%) exceeds 20%"
        )
    if ci_cv > 10:
        stability_issues.append(f"C-index CV ({ci_cv:.1f}%) exceeds 10%")
    if sig_count < total_count:
        stability_issues.append(
            f"Only {sig_count}/{total_count} seeds achieved significance"
        )
    if ph_valid < total_count * 0.8:
        stability_issues.append(
            f"PH assumption violated in {total_count - ph_valid}/{total_count} seeds"
        )
